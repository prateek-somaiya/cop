...
const fetchComputeOptimizerRecommendationForAccount = async (account: string, region: string) =>
  (
    ...
    (
      await Promise.allSettled(
        (accountIds && accountIds.length > 0
          ...
          (
            await Promise.allSettled(
              regions.flatMap(region => fetchComputeOptimizerRecommendationForAccount(accountId, region)),
            )
          )
